...
export const MemorialsPage: React.FC = () => {
  ...
  const confirmDelete = async () => {
    ...
    await MemorialService.deleteMemorial(deleteTargetId, currentFamId);
    ...
    <div className="space-y-6 animate-fade-in font-sans">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 relative overflow-hidden">
        ...
        <div className="flex items-center gap-2 flex-wrap shrink-0">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2.5 bg-[#166534] hover:bg-[#14532d] dark:bg-emerald-700 dark:hover:bg-emerald-600 text-white text-xs font-bold rounded-xl transition shadow-sm cursor-pointer"
          >
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full text-xs font-bold transition-all cursor-pointer ${
              filterMode === 'BRANCH'
                ? 'bg-[#2E1E6B] dark:bg-emerald-700 text-white shadow-sm'
                : 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700'
            }`}
          >
            Theo Chi
        ...
        <div className="flex items-center gap-3 flex-1 min-w-[280px] justify-end">
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 text-xs bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-[#166534] focus:bg-white dark:focus:bg-slate-800 transition"
            />
          </div>

          <div className="flex items-center space-x-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-xs text-slate-700 dark:text-slate-200">
            <Filter className="w-3.5 h-3.5 text-slate-400" />
            <select
              value={selectedMonth}
              onChange={(e) => setSelectedMonth(e.target.value)}
              className="bg-transparent focus:outline-none font-bold text-slate-800 dark:text-white"
            >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredMemorials.length === 0 ? (
          <div className="col-span-2 py-16 text-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6">
            <Sparkles className="w-8 h-8 text-amber-400 mx-auto mb-2" />
            <p className="text-sm font-bold text-slate-700 dark:text-slate-200">Không tìm thấy ngày giỗ phù hợp</p>
          ...
          filteredMemorials.map((mem) => {
            ...
            return (
              <div
                key={mem.id}
                className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-amber-400 dark:hover:border-amber-600 transition p-5 flex flex-col justify-between group"
              >
                <div>
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center space-x-3">
                      <div className="w-12 h-12 rounded-2xl bg-amber-50 dark:bg-amber-950/60 border border-amber-200 dark:border-amber-800 flex items-center justify-center text-amber-800 dark:text-amber-300 font-bold shadow-sm">
                        <Sparkles className="w-6 h-6" />
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <h2 className="text-sm font-bold text-slate-900 dark:text-white group-hover:text-amber-900 dark:group-hover:text-amber-300 transition-colors">
                            {mem.title}
                          </h2>
                          {isAutoSynced && (
                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-50 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-300 font-bold border border-emerald-200 dark:border-emerald-800">
                              Tự động
                            </span>
                          )}
                        </div>
                        <div className="text-xs text-slate-500 dark:text-slate-400 font-medium mt-0.5">
                          {member?.full_name} • <span className="text-slate-700 dark:text-slate-200 font-semibold">{branchName}</span> ({genName})
                        </div>
                      </div>
                    </div>

                    <div className="flex flex-col items-end gap-1">
                      <span className="text-xs font-bold bg-amber-100 dark:bg-amber-950/80 text-amber-900 dark:text-amber-200 px-3 py-1 rounded-full border border-amber-200 dark:border-amber-700 shadow-xs">
                        Ngày {mem.lunar_day}/{mem.lunar_month} ÂL
                      </span>
                      {mem.is_leap_month && (
                        <span className="text-[10px] font-bold bg-purple-100 dark:bg-purple-950 text-purple-900 dark:text-purple-300 px-2 py-0.5 rounded-md border border-purple-200 dark:border-purple-800">
                          Tháng Nhuận
                        </span>
                      )}
                    </div>
                  ...
                  {/* Date & Location */}
                  <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                    <div className="flex items-center space-x-2 text-slate-600 dark:text-slate-300">
                      <Calendar className="w-3.5 h-3.5 text-[#166534] dark:text-emerald-400 shrink-0" />
                      <span>
                        Dương lịch dự kiến: <strong className="text-slate-900 dark:text-white font-bold">{formatDate(mem.next_solar_date)}</strong>
                      </span>
                    </div>
                    {member?.burial_place && (
                      <div className="flex items-center space-x-2 text-slate-600 dark:text-slate-300">
                  ...
                  {is30thDay && (
                    <div className="mt-3 p-2 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-xl text-[11px] text-amber-900 dark:text-amber-200 flex items-center space-x-1.5">
                  ...
                  {mem.notes && (
                    <p className="text-[11px] text-slate-600 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/80 p-2.5 rounded-xl mt-3 border border-slate-100 dark:border-slate-700">
                      {mem.notes}
                    </p>
                  )}
                </div>
                ...
                <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs">
                  <span className="text-[11px] font-semibold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/60 px-2.5 py-0.5 rounded-full border border-emerald-100 dark:border-emerald-800">
                  ...
                  <div className="flex items-center space-x-2">
                    {member && (
                      <button
                        ...
                        className="px-2.5 py-1 bg-amber-100/80 dark:bg-amber-950/80 hover:bg-amber-200 dark:hover:bg-amber-900 border border-amber-300 dark:border-amber-700 text-amber-950 dark:text-amber-300 font-bold rounded-lg text-xs flex items-center gap-1 transition cursor-pointer"
                      >
                        <ScrollText className="w-3.5 h-3.5 text-amber-800 dark:text-amber-400" />
                        <span>Văn Khấn</span>
                      </button>
                    )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-sm w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-white">Xác Nhận Xóa</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">Bạn có chắc chắn muốn xóa ngày giỗ này khỏi danh bạ gia tộc?</p>
              </div>
            </div>
