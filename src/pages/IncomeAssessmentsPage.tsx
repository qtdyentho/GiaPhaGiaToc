    ...
    <div className="space-y-6 animate-fade-in font-sans">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-wrap items-center justify-between gap-4 relative overflow-hidden">
        ...
        <div className="flex items-center gap-3.5">
          <div className="w-12 h-12 rounded-2xl bg-amber-50 dark:bg-amber-950/60 border border-amber-200 dark:border-amber-800 flex items-center justify-center text-amber-800 dark:text-amber-300 shadow-sm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/90 dark:border-slate-800 shadow-sm p-5">
          <div className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Tổng Nghĩa Vụ Phải Thu</div>
          <div className="text-2xl font-black text-slate-900 dark:text-white mt-1">{totalDue.toLocaleString()} ₫</div>
          <div className="text-xs text-slate-400 mt-0.5">{assessments.length} suất nghĩa vụ thu</div>
        </div>

        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/90 dark:border-slate-800 shadow-sm p-5">
          <div className="text-xs font-bold text-emerald-800 dark:text-emerald-400 uppercase tracking-wider">Đã Thực Thu (Vào Quỹ)</div>
          <div className="text-2xl font-black text-emerald-800 dark:text-emerald-400 mt-1">{totalPaid.toLocaleString()} ₫</div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/90 dark:border-slate-800 shadow-sm p-5">
          <div className="text-xs font-bold text-amber-700 dark:text-amber-400 uppercase tracking-wider">Còn Tồn Đọng Chưa Thu</div>
          <div className="text-2xl font-black text-amber-700 dark:text-amber-400 mt-1">{totalPending.toLocaleString()} ₫</div>
          <div className="text-xs text-slate-400 mt-0.5">
      ...
      <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-wrap items-center justify-between gap-3">
        <div className="relative flex-1 min-w-[240px]">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 text-xs bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-1 focus:ring-[#166534] focus:bg-white dark:focus:bg-slate-800 transition"
          />
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/90 dark:border-slate-800 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-700 dark:text-slate-300">
            <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 text-[11px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wider">
              <tr>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {loading ? (
                ...
                filteredAssessments.map((asm) => {
                  ...
                  return (
                    <tr key={asm.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition">
                      <td className="py-3.5 px-4 font-bold text-slate-900 dark:text-white">{asm.title}</td>
                      <td className="py-3.5 px-4 font-semibold text-slate-800 dark:text-slate-200">{member?.full_name || 'Thành viên'}</td>
                      <td className="py-3.5 px-4 text-right font-bold text-slate-900 dark:text-white">
                        {Number(asm.amount_due).toLocaleString()} ₫
                      </td>
                      <td className="py-3.5 px-4 text-right font-bold text-emerald-800 dark:text-emerald-400">
                        {Number(asm.amount_paid).toLocaleString()} ₫
                      </td>
                      <td className="py-3.5 px-4 text-slate-600 dark:text-slate-400">{asm.due_date || '2026-12-31'}</td>
                      <td className="py-3.5 px-4 text-center">
                        <span
                          className={`font-bold px-2.5 py-0.5 rounded-full text-[10px] ${
                            isPaid
                              ? 'bg-emerald-50 dark:bg-emerald-950/60 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800'
                              : 'bg-amber-50 dark:bg-amber-950/60 text-amber-900 dark:text-amber-300 border border-amber-300 dark:border-amber-700 font-bold'
                          }`}
                        >
                      ...
                      <td className="py-3.5 px-4 text-right">
                        {!isPaid ? (
                          <button
                            ...
                            className="px-3 py-1.5 bg-[#166534] hover:bg-[#14532d] dark:bg-emerald-700 dark:hover:bg-emerald-600 text-white font-bold rounded-xl text-xs transition shadow-xs inline-flex items-center gap-1 cursor-pointer"
                          >
                            <ArrowDownLeft className="w-3.5 h-3.5" />
                            <span>Ghi Thu Tiền</span>
                          </button>
                        ) : (
                          <span className="text-emerald-700 dark:text-emerald-400 font-semibold text-xs flex items-center justify-end gap-1">
