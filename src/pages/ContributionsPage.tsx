...
export const ContributionsPage: React.FC = () => {
  ...
  const exportCSV = () => {
    const headers = ['Người Đóng Góp', 'Quỹ', 'Mục Đích', 'Phương Thức', 'Số Tiền', 'Ngày'];
    const rows = filteredContributions.map((c) => [
      c.donor_name || 'Nhà hảo tâm',
      funds.find((f) => f.id === c.fund_id)?.name || 'Quỹ',
      `"${(c.purpose || '').replace(/"/g, '""')}"`,
      c.payment_method,
      c.amount,
    ...
    link.setAttribute('download', `cong_duc_dong_gop_${new Date().toISOString().slice(0, 10)}.csv`);
    ...
    <div className="space-y-6 animate-fade-in font-sans">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-wrap items-center justify-between gap-4 relative overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={exportCSV}
            className="px-3.5 py-2.5 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 text-xs font-semibold flex items-center gap-1.5 shadow-sm transition"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 shadow-sm">
          <span className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Tổng Tiền Đóng Góp</span>
        ...
        <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 shadow-sm">
          <span className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Số Lượng Đóng Góp</span>
          <p className="text-2xl font-bold text-slate-900 dark:text-white mt-1">{contributions.length} lượt</p>
        </div>

        <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 shadow-sm">
          <span className="text-xs text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wider">Bình Quân / Lượt</span>
        ...
        <div className="flex items-center gap-3 flex-1 min-w-[280px] max-w-md">
          <div className="relative w-full">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 text-xs focus:outline-none focus:ring-1 focus:ring-[#166534] focus:bg-white dark:focus:bg-slate-900 transition"
            />
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-700">
            <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 text-[11px] uppercase tracking-wider text-slate-600 dark:text-slate-400 font-bold">
              <tr>
                    ...
                    <tr key={c.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                      <td className="py-4 px-4 font-medium text-slate-900">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div>
                            <p className="font-bold text-slate-900 dark:text-white">{c.donor_name || 'Nhà hảo tâm'}</p>
                      ...
                      <td className="py-4 px-4 text-slate-600 dark:text-slate-300 max-w-sm">{c.purpose}</td>

                      <td className="py-4 px-4">
                        <span className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-[11px] font-semibold inline-flex items-center gap-1">
                          {c.payment_method === 'VIETQR' ? (
                            <QrCode className="w-3 h-3 text-emerald-700 dark:text-emerald-400" />
                          ) : (
                      ...
                      <td className="py-4 px-4 text-right font-bold text-base text-amber-800 dark:text-amber-400">
