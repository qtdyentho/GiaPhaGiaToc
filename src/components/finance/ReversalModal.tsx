      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-amber-50 via-slate-50 to-white dark:from-slate-800 dark:via-slate-800/90 dark:to-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-900 dark:text-white">Hoàn Trả Bút Toán Thu Chi</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">Ghi nhận phiếu hoàn trả & bảo toàn lịch sử sổ quỹ</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 space-y-2 text-xs">
            <div className="flex items-center justify-between">
              <span className="text-slate-500 dark:text-slate-400">Mã chứng từ:</span>
              <span className="font-bold text-slate-900 dark:text-white">{transaction.transaction_code}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-slate-500 dark:text-slate-400">Loại giao dịch:</span>
              <span
                className={`font-semibold ${
                  transaction.transaction_type === 'INCOME' ? 'text-emerald-700 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'
                }`}
              >
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-bold text-slate-900 dark:text-white">{Number(transaction.amount).toLocaleString()} ₫</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-slate-500 dark:text-slate-400">Quỹ:</span>
              <span className="text-slate-900 dark:text-white">{fund?.name || 'Quỹ gia tộc'}</span>
            </div>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-amber-50 dark:bg-amber-950/60 border border-amber-200 dark:border-amber-800 text-xs text-amber-900 dark:text-amber-200 flex items-start gap-2">
            ...
            <span>
              <strong>Nguyên tắc minh bạch:</strong> Chứng từ gốc được lưu giữ nguyên vẹn để đối soát. Hệ thống sẽ tạo một phiếu hoàn trả đối ứng <code className="bg-amber-100 dark:bg-amber-900/60 px-1 py-0.5 rounded text-amber-950 dark:text-amber-100">REV-{transaction.transaction_code}</code> để cân bằng số dư quỹ chính xác.
            </span>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-3.5 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 text-xs focus:outline-none focus:border-amber-500"
              required
            />
          </div>
          ...
          <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-5 py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 dark:bg-amber-600 dark:hover:bg-amber-500 text-slate-950 dark:text-white text-xs font-bold flex items-center gap-1.5 shadow-sm disabled:opacity-50 transition-all cursor-pointer"
            >
              {loading ? (
