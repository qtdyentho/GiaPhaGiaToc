...
test('12-COLUMN EXCEL GENEALOGY IMPORT & PARSER TEST SUITE', async (t) => {
  ...
  await t.test('IMPORT-003: Validation Logic & Loop Detection Guard', () => {
    const sampleRows: RawImportMember[] = [
      ...
      },
      {
        ...
        generationNumber: 3,
        branchName: 'Chi Trưởng',
        parentName: 'Cụ Nguyễn Văn Khang',
        lifeStatus: 'ALIVE',
        birthYear: 1975
      },
      {
    ...
    console.log('✅ [IMPORT-003: Validation Logic & Loop Detection] PASS');
  });

  await t.test('IMPORT-004: Clean Data Validation Passes with High Confidence', () => {
    const validRows: RawImportMember[] = [
      {
        fullName: 'Cụ Nguyễn Văn Phúc',
        gender: 'MALE',
        generationNumber: 1,
    ...
    const validation = DataImportService.validateImportData(validRows);
    assert.strictEqual(validation.errorRows, 0);
    assert.strictEqual(validation.canCommit, true);
    assert.strictEqual(validation.validRows, 2);
    console.log('✅ [IMPORT-004: Clean Data Validation] PASS');
  });
